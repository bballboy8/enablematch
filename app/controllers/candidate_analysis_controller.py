...
async def generate_metadata_for_candidates(background_tasks: BackgroundTasks, request: GenerateMetadataForCandidatesRequestBody, user_id: str = Depends(get_current_user_id)):
    ...
    background_tasks.add_task(candidate_analysis_service.generate_metadata_of_candidates, job_description=request.job_description, compensation_range=request.compensation_range, location=request.location, contractors_only=request.contractors_only)
...
async def get_ai_matrix_by_trigger_id(trigger_id: str, page:int = 1, page_size: int = 10, sort_by: str = "final_score", min_score: float = 0, user_id: str = Depends(get_current_user_id)):
    """Get the AI matrix by trigger ID."""
    logger.info("Get AI matrix by trigger ID entry point")
    response = await candidate_analysis_service.get_ai_matrix_by_trigger_id(trigger_id, page=page, page_size=page_size, sort_by=sort_by, min_score=min_score)
