...
async def get_ai_matrix_by_trigger_id(trigger_id: str, page:int = 1, page_size: int = 10, sort_by: str = "final_score", user_id: str = Depends(get_current_user_id)):
    """Get the AI matrix by trigger ID."""
    logger.info("Get AI matrix by trigger ID entry point")
    response = await candidate_analysis_service.get_ai_matrix_by_trigger_id(trigger_id, page=page, page_size=page_size, sort_by=sort_by)
