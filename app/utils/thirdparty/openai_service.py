...
class OpenAIService:
    ...
    async def _build_metadata_v2_messages(self, text_blob: str) -> tuple[str, str]:
        system_prompt, prompt, _ = await pipeline_prompt_config.render_pipeline_prompt(
            constants.METADATA_GENERATION_PROMPT_KEY,
            text_blob=text_blob,
        )
        return system_prompt, prompt

    async def _build_relevant_experience_years_v2_messages(self, experience_text: str, job_description: str) -> tuple[str, str]:
        system_prompt, prompt, _ = await pipeline_prompt_config.render_pipeline_prompt(
    ...
    async def _build_select_candidates_for_matching_messages(self, job_description: str, candidate_summary: str, compensation_range: str, location: str, candidates_current_location: str, candidates_compensation_range: str) -> tuple[str, str]:
        system_prompt, prompt, _ = await pipeline_prompt_config.render_pipeline_prompt(
            ...
            location=location,
            candidates_current_location=candidates_current_location,
            candidates_compensation_range=candidates_compensation_range,
        )
        return system_prompt, prompt
    ...
    async def generate_metadata_via_ai_v2(self, text_blob: str):
        ...
        """
        try:
            system_prompt, prompt = await self._build_metadata_v2_messages(text_blob)
    ...
    async def generate_relevant_experience_years_v2(self,
        ...
        """
        try:
            system_prompt, prompt = await self._build_relevant_experience_years_v2_messages(experience_text, job_description)
    ...
    async def select_candidates_for_matching(self, job_description: str, candidate_summary: str, compensation_range: str, location: str, candidates_current_location: str, candidates_compensation_range: str):
        try:
            system_prompt, prompt = await self._build_select_candidates_for_matching_messages(
                job_description,
                candidate_summary,
                compensation_range,
                location,
    ...
    async def generate_relevant_experience_years_v2_batch(self, candidates: list[dict], stop_checker=None) -> dict:
        ...
        for candidate in candidates:
            system_prompt, prompt = await self._build_relevant_experience_years_v2_messages(
    ...
    async def generate_metadata_via_ai_v2_batch(self, candidates: list[dict], stop_checker=None) -> dict:
        requests = []
        for candidate in candidates:
            system_prompt, prompt = await self._build_metadata_v2_messages(candidate["text_blob"])
            requests.append(self._build_chat_completion_request(candidate["custom_id"], system_prompt, prompt))
